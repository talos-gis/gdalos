...
def viewshed_calc(input_ds,
    ...
    if not files:
        ...
        for vp in arrays_dict:
            ...
            d_path = tempfile.mktemp(
                suffix=ext) if (use_temp_tif and steps>1) else output_filename if gdal_out_format != 'MEM' else ''
    ...
    if operation:
        ...
        ds = gdal_calc.Calc(
            ...
            color_table=color_table, hideNodata=hide_nodata, return_ds=gdal_out_format == 'MEM', overwrite=True, **kwargs)
        # if ds is None:
        #     raise Exception('error occurred')
